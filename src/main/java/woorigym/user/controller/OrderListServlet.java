...
public class OrderListServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 총 글수
		bCount = new OrderListService().getOrderCount(userTableSessionAttr.getUser_id());
		...
		ArrayList<OrderList> volist = new OrderListService().selectOrderList(userTableSessionAttr.getUser_id(),startRnum,endRnum);
...
//			out.println("  다음");
		...
		request.setAttribute("ordervolist", volist);
		request.setAttribute("startPage", startPage);
		request.setAttribute("endPage", endPage);
		request.setAttribute("pageCount", pageCount);
		
		
		//Page 이동하면서도 Data도 전닳함
